...
count_ok = 0
count_not_ok = 0
for line in open('/data/click_bait_detect/targetTitle_cleaned_splitted_glove_vectors','r'):
    if len(eval(line))>0:
        count_ok = count_ok + 1
...
features_sent_sub.shape #(21980, 50)
...
title_features = pd.DataFrame(features_sent_sub)
title_features.head()
...
da = pd.concat([data_features_label_1,title_features], axis = 1)
